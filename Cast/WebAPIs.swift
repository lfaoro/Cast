...
final class WebAPIs: NSObject {
	...
	*/
	func startShorten(URL: String, success: (NSURL) -> ()) {
		...
		let bitlyAPIshorten = bitlyAPIurl + "/v3/shorten?access_token=" +
			bitlyOAuth2Token + "&longUrl=" + URL
		...
		session.dataTaskWithURL(url) { (data, response, error) in
			if let data = data {
				...
				app.userNotification.pushNotification(error: "bit.ly Unreachable",
	...
	*/
	func startUpload(data: PasteboardData, fileName: String = "Casted.swift",
		isPublic: Bool = true, success: (String) -> ()) {
		//---------------------------------------------------------------------------
		var jsonBody: JSON!
		...
		case .Image(_):
			...
			return
			//---------------------------------------------------------------------------
		default: app.userNotification.pushNotification(error: "Service Unreachable",
			description: "Your pasteboard contains invalid data.")
		}
		//---------------------------------------------------------------------------
		session.dataTaskWithRequest(request) { (data, response, error) in
			if let data = data {
				...
				print(error!.localizedDescription)
				app.userNotification.pushNotification(error: "Service Unreachable",
					description: error!.localizedDescription)
			}
			}.resume()
		//---------------------------------------------------------------------------
